    ...
    <div className="space-y-1">
      {events.map((e) => (
        <button key={e.id} onClick={() => onSelect(e)} style={{ marginLeft: depthOf(e, events) * 16 }}
          className={`w-full text-left border-l-4 ${COLORS[e.type] ?? 'border-gray-200'} ${e.span_id && e.span_id !== 'loop' && e.span_id !== 'supervisor' ? 'bg-amber-50/40' : 'bg-white'} p-2 rounded hover:bg-gray-50`}>
          <span className="font-mono text-xs text-gray-500">seq {e.seq}</span>{' '}
          <span className="font-mono text-sm">{e.type}</span>{' '}
          {e.span_id && e.span_id !== 'loop' && e.span_id !== 'supervisor' && (
