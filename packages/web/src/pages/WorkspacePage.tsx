...
export function WorkspacePage() {
  ...
  function move(event: PointerEvent, id: string) { if (drag !== id) return; const box = event.currentTarget.parentElement?.getBoundingClientRect(); if (!box) return; const x = Math.max(4, Math.min(96, ((event.clientX-box.left)/box.width)*100)); const y = Math.max(8, Math.min(92, ((event.clientY-box.top)/box.height)*100)); setNodes((all) => all.map((n) => n.id === id ? { ...n, x, y } : n)); }
  return <div className="agent-workspace"><header className="workspace-heading"><div><div className="eyebrow">AGENT STUDIO / 工作区</div><h1 className="page-title">研究助手</h1><p className="page-desc">在画布上组织 Agent 的能力，运行后再到回放工作台检查每一步。</p></div><div className="workspace-actions"><button className="btn btn-ghost">保存草稿</button><button className="btn btn-primary">运行测试</button></div></header>
    ...
    <div className="canvas-layout"><section className="agent-canvas" aria-label="Agent 画布" onPointerMove={(e)=>drag&&move(e,drag)} onPointerUp={()=>setDrag(null)}><svg className="canvas-lines" viewBox="0 0 1000 600" preserveAspectRatio="none" aria-hidden="true"><path d="M190 300 C300 300 320 170 420 170"/><path d="M190 300 C300 300 320 430 420 430"/><path d="M580 170 C680 170 700 300 810 300"/><path d="M580 430 C680 430 700 300 810 300"/></svg>{nodes.map(n=><button type="button" key={n.id} className={`canvas-node node-${n.tone} ${selected===n.id?'is-selected':''}`} style={{left:`${n.x}%`,top:`${n.y}%`}} onPointerDown={()=>setDrag(n.id)} onClick={()=>setSelected(n.id)}><span className="canvas-node-kicker">{n.label}</span><strong>{n.title}</strong><small>{n.meta}</small></button>)}<div className="canvas-zoom"><button>−</button><span>100%</span><button>＋</button></div></section><aside className="node-inspector"><div className="inspector-kicker">SELECTED NODE</div><h2>{node.title}</h2><p>{node.meta}</p><div className="inspector-rule"/><label>节点类型<span>{node.label}</span></label><label>状态<span className="inspector-ok">● 已连接</span></label><label>输入与输出<span>完整记录</span></label><button className="btn btn-ghost inspector-edit">编辑节点配置</button><a className="inspector-link" href="/specs">打开高级 Spec 编辑 →</a></aside></div>
  </div>;
}
