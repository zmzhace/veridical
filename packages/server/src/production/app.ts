...
export async function buildProductionApp(options: {
  ...
  app.get('/v1/models', async (req) => {
    ...
    await db.audit(req.principal.tenant, req.principal.actor, 'models.read', {
      count: models.length,
      request_id: req.id,
    });
    return models;
  ...
  app.get('/v1/tools', async (req) => {
    ...
    await db.audit(req.principal.tenant, req.principal.actor, 'tools.read', {
      count: tools.length,
      request_id: req.id,
    });
    return tools;
  ...
  app.post('/v1/replays', async (req, reply) => {
    ...
    return reply.code(202).send(
      jobView(
        await service.replay(req.principal, body.session, idem(req.headers), {
  ...
  app.post('/v1/replay', async (req, reply) => {
    const body = z
      .object({
        session: Key,
        // Production replay is deliberately strict-only; fixture/semantic are research APIs.
