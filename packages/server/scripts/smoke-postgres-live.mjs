...
if (!baseUrl || !model || !apiKey)
  throw new Error(
    'VERIDICAL_PROVIDER_KEY, VERIDICAL_LLM_BASE_URL and VERIDICAL_LLM_MODEL are required',
  );
const postgres =
...
const tokenValues = Object.fromEntries(
  ['developer', 'reviewer', 'publisher', 'operator'].map((role) => [
    role,
    randomBytes(32).toString('hex'),
  ]),
);
const token = (role) => ({
  hash: createHash('sha256').update(tokenValues[role]).digest('hex'),
  tenant: 'live',
  actor: role,
  roles: [role],
...
const config = ProductionConfigSchema.parse({
  database: '/tmp/veridical-live-unused.db',
  releaseId: 'qwen-live-e2e-release',
  dataKeyEnv: 'LIVE_DATA',
  auditKeyEnv: 'LIVE_AUDIT',
  storage: {
    database: 'postgres',
    queue: 'redis',
    objectStore: 's3',
    ...
    redisUrl: redis,
    s3Endpoint,
    s3Bucket: process.env.VERIDICAL_S3_BUCKET ?? 'veridical-artifacts',
    s3AccessKeyEnv: 'LIVE_S3_ACCESS',
    s3SecretKeyEnv: 'LIVE_S3_SECRET',
  },
  tokens: ['developer', 'reviewer', 'publisher', 'operator'].map(token),
  providers: [
    {
      name: 'qwen',
      model,
      version: 'live-configured',
      baseUrl,
      apiKeyEnv: 'VERIDICAL_PROVIDER_KEY',
    },
  ],
  timeoutMs: Number(process.env.VERIDICAL_LIVE_TIMEOUT_MS ?? 120000),
  maxOutputTokens: Number(process.env.VERIDICAL_LLM_MAX_OUTPUT_TOKENS ?? 512),
  requestsPerMinute: 1000,
});
const provider = new SecureProvider(baseUrl, apiKey, model, {
  enableThinking: process.env.VERIDICAL_LLM_ENABLE_THINKING === 'true',
});
const queue = new RedisJobQueue(redis, `veridical:live:${Date.now()}`);
const env = await buildProductionApp({
  config,
  dataKey: randomBytes(32),
  auditKey: randomBytes(32),
  providers: new Map([['qwen', provider]]),
  tools: safeTools,
  asyncJobs: queue,
  logger: false,
});
const request = (role, method, url, payload, idem) =>
  env.app.inject({
    method,
    url,
    payload,
    headers: {
      authorization: `Bearer ${tokenValues[role]}`,
...
async function waitJob(id) {
  ...
  throw new Error(`live job timeout: ${id}`);
}
...
try {
  ...
  if (specResponse.statusCode !== 201)
    console.error('live spec rejected', specResponse.statusCode, specResponse.body);
  assert.equal(specResponse.statusCode, 201);
  assert.equal(
    (
      await request('reviewer', 'POST', `/v1/suites/${name}`, {
        name: 'acceptance',
        cases: [1, 2, 3].map((i) => ({ input: `live-${i}`, contains: ['LIVE_OK'] })),
      })
    ).statusCode,
    201,
  );
  ...
  assert.equal(
    (
      await request('reviewer', 'POST', '/v1/approvals', {
        ref: `${name}@1.0.0`,
        reason: 'live independent review',
      })
    ).statusCode,
    200,
  );
  assert.equal(
    (
      await request('publisher', 'POST', `/v1/deployments/${name}`, {
        ref: `${name}@1.0.0`,
        channel: 'production',
        reason: 'live canary acceptance',
      })
    ).statusCode,
    200,
  );
