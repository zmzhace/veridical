...
const tokens = {
  developer: randomBytes(32).toString('hex'),
  reviewer: randomBytes(32).toString('hex'),
  publisher: randomBytes(32).toString('hex'),
...
const config = ProductionConfigSchema.parse({
  database: '/tmp/veridical-postgres-e2e-unused.db',
  releaseId: 'postgres-e2e-release',
  dataKeyEnv: 'E2E_DATA',
  auditKeyEnv: 'E2E_AUDIT',
  storage: {
    database: 'postgres',
    objectStore: 's3',
    queue: 'redis',
    ...
    redisUrl: redis,
    s3Endpoint: process.env.VERIDICAL_S3_ENDPOINT ?? 'http://127.0.0.1:9000',
    s3Bucket: process.env.VERIDICAL_S3_BUCKET ?? 'veridical-artifacts',
    s3AccessKeyEnv: 'E2E_S3_ACCESS',
    s3SecretKeyEnv: 'E2E_S3_SECRET',
  },
  requestsPerMinute: 1000,
  tokens: Object.entries(tokens).map(([role, value]) => ({
...
const request = (method, url, payload, idempotency, role = 'developer') =>
  env.app.inject({
    method,
    url,
    payload,
...
try {
  ...
  if (specResponse.statusCode !== 201)
    console.error('spec response', specResponse.statusCode, specResponse.body);
  assert.equal(specResponse.statusCode, 201);
  const suite = {
    name: 'acceptance',
    cases: [1, 2, 3].map((i) => ({ input: `case-${i}`, contains: ['E2E OK'] })),
  };
  assert.equal(
    (await request('POST', `/v1/suites/${agentName}`, suite, undefined, 'reviewer')).statusCode,
    201,
  );
  ...
  assert.equal(
    (
      await request(
        'POST',
        '/v1/approvals',
        { ref: `${agentName}@1.0.0`, reason: 'independent e2e approval' },
        undefined,
        'reviewer',
      )
    ).statusCode,
  ...
  assert.equal(
    (
      await request(
        'POST',
        `/v1/deployments/${agentName}`,
        { ref: `${agentName}@1.0.0`, channel: 'production', reason: 'e2e rollout' },
        undefined,
        'publisher',
      )
    ).statusCode,
  ...
  const run = await request(
    'POST',
    '/v1/runs',
    { name: agentName, channel: 'production', prompt: 'hello' },
    `e2e-run-${agentName}`,
    'operator',
  );
  ...
  const artifactResponse = await request(
    'GET',
    `/v1/artifacts/spec/${agentName}@1.0.0`,
    undefined,
    undefined,
    'operator',
  );
  ...
  const contentResponse = await request(
    'GET',
    `/v1/artifacts/spec/${agentName}@1.0.0/content`,
    undefined,
    undefined,
    'operator',
  );
  ...
  const fileResponse = await request(
    ...
    '/v1/knowledge/files',
    {
      ...
      content_base64: Buffer.from('PostgreSQL S3 knowledge assertion').toString('base64'),
    },
    undefined,
    'developer',
  );
  ...
  assert.equal(
    (
      await request(
        'GET',
        `/v1/knowledge/files/${file.id}/content`,
        undefined,
        undefined,
        'operator',
      )
