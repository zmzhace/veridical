...
if (!source || !url || !dataKey || !auditKey)
  throw new Error('VERIDICAL_SQLITE_PATH, VERIDICAL_POSTGRES_URL, VERIDICAL_DATA_KEY and VERIDICAL_AUDIT_KEY are required');
...
const report = {
  ...
  counts: { sessions: 0, events: 0, artifacts: 0, pointers: 0, jobs: 0, revoked_tokens: 0, rate_limits: 0 },
  tenants: [],
  failures: [],
};
...
async function importAll() {
  ...
  try {
    ...
    const sessions = sqlite.sql.prepare('SELECT tenant,id,kind,ref,created,seq,head FROM sessions ORDER BY tenant,created').all();
    ...
    for (const row of sessions) {
      ...
      const events = sqlite.sql.prepare('SELECT tenant,session,seq,id,blob,prev,hash FROM events WHERE tenant=? AND session=? ORDER BY seq').all(row.tenant, row.id);
    ...
    for (const row of sqlite.sql.prepare('SELECT tenant,kind,key,author,status,digest,blob,meta,seal,created FROM artifacts ORDER BY tenant,kind,key').all()) {
      tenantSet.add(row.tenant);
      sourceHash.update(JSON.stringify(row));
      await client.query(
        `INSERT INTO artifacts(tenant,kind,key,author,status,digest,blob,meta,seal,created) VALUES($1,$2,$3,$4,$5,$6,$7,$8::jsonb,$9,$10)`,
        [row.tenant, row.kind, row.key, row.author, row.status, row.digest, row.blob, JSON.stringify(row.meta), row.seal, row.created],
      );
      report.counts.artifacts++;
    }
    for (const row of sqlite.sql.prepare('SELECT tenant,kind,name,ref,seal FROM pointers ORDER BY tenant,kind,name').all()) {
      tenantSet.add(row.tenant);
      sourceHash.update(JSON.stringify(row));
      await client.query(`INSERT INTO pointers(tenant,kind,name,ref,seal) VALUES($1,$2,$3,$4,$5)`, [row.tenant, row.kind, row.name, row.ref, row.seal]);
      report.counts.pointers++;
    }
    for (const row of sqlite.sql.prepare('SELECT tenant,id,actor,session,kind,idem,request_hash,blob,state,owner,deadline,lease_until,result,created FROM jobs ORDER BY tenant,created').all()) {
      tenantSet.add(row.tenant);
      sourceHash.update(JSON.stringify(row));
      await client.query(
        ...
        [row.tenant, row.id, row.actor, row.session, row.kind, row.idem, row.request_hash, row.blob, row.state, row.owner, row.deadline, row.lease_until, row.result === null ? null : JSON.stringify(row.result), row.created],
      );
      report.counts.jobs++;
    }
    ...
    for (const row of sqlite.sql.prepare('SELECT key,window,count FROM rate_limits').all()) {
      sourceHash.update(JSON.stringify(row));
      await client.query('INSERT INTO rate_limits(key,window_start,count) VALUES($1,$2,$3)', [row.key, row.window, row.count]);
...
async function verifyTarget() {
  ...
  try {
    ...
    if (sessions.length !== report.counts.sessions) throw new Error('migration_session_count_mismatch');
    ...
    for (const [name, value] of Object.entries(result.rows[0]))
      if (Number(value) !== report.counts[name]) throw new Error(`migration_${name}_count_mismatch`);
  } catch (error) {
...
async function rollbackMigration() {
  ...
  try {
    await client.query('BEGIN');
    await client.query('SELECT pg_advisory_xact_lock($1)', [71938402]);
    const result = await client.query('SELECT * FROM migration_checkpoints WHERE id=$1 FOR UPDATE', [id]);
    ...
    await client.query('CREATE TRIGGER events_no_update BEFORE UPDATE ON events FOR EACH ROW EXECUTE FUNCTION reject_event_mutation()');
    await client.query('CREATE TRIGGER events_no_delete BEFORE DELETE ON events FOR EACH ROW EXECUTE FUNCTION reject_event_mutation()');
    await client.query(`UPDATE migration_checkpoints SET status='rolled_back',completed_at=now() WHERE id=$1`, [id]);
    await client.query('COMMIT');
