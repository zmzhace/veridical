...
const run = (args, env) =>
  new Promise((resolve, reject) => {
    const child = spawn(
      process.execPath,
      ['--import', 'tsx', 'scripts/migrate-postgres.mjs', ...args],
    ...
    child.on('close', (code) =>
      code === 0
        ? resolve({ out, err })
        : reject(new Error(`migration exited ${code}: ${err || out}`)),
    );
  });
const env = {
  ...process.env,
  VERIDICAL_SQLITE_PATH: source,
  VERIDICAL_POSTGRES_URL: postgres,
  VERIDICAL_DATA_KEY: dataKey.toString('hex'),
  VERIDICAL_AUDIT_KEY: auditKey.toString('hex'),
  VERIDICAL_MIGRATION_REPORT: report,
  VERIDICAL_MIGRATION_ID: `smoke-${randomUUID()}`,
};
await run([], env);
