...
export async function runStageGate(
  ctx: FlowContext, prompt: string, stages: Stage[],
  ...
  if (turn) {
    ...
    await ctx.recorder.record({ span_id: 'loop', parent_span_id: null, type: 'user.message', verb: 'request', attempt: 1, duration_ms: 0, payload: { text: prompt } });
  }
  const history = await readEvents();
  const turnStartSeq = history.reduce((m, e) => Math.max(m, e.seq), 0);
  const completed = new Set(history.filter(e => e.type === 'stage/end' && e.verb === 'response').map(e => (e.payload as any).stage));
  const startedStages = new Set(history.filter(e => e.type === 'stage/start').map(e => (e.payload as any).stage));
